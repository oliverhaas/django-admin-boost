...
class RangeNumericMixin(ListFilter):
    ...
    def init_used_parameters(self, params: dict[str, Any]) -> None:
        if f"{self.parameter_name}_from" in params:
            ...
            self.used_parameters[f"{self.parameter_name}_from"] = value[0] if isinstance(value, list) else value

        if f"{self.parameter_name}_to" in params:
            value = params.pop(f"{self.parameter_name}_to")
            self.used_parameters[f"{self.parameter_name}_to"] = value[0] if isinstance(value, list) else value
    ...
    def choices(self, changelist: ChangeList) -> Iterator:
        yield {
            "form": self.form_class(
                ...
                multiple=self.multiple if hasattr(self, "multiple") and isinstance(self.multiple, bool) else False,
            ),
        }
