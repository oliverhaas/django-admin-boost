...
from django_adminx.exceptions import NotRegistered
from django_adminx.utils import NotRelationField, flatten, get_fields_from_path
...
def check_admin_app(app_configs, **kwargs):
    from django_adminx.sites import all_sites
...
def check_dependencies(**kwargs):
    ...
    if not apps.is_installed("django_adminx"):
        return []
    errors = []
    app_dependencies = (
...
class BaseModelAdminChecks:
        ...
        if duplicate_fields := [field for field, count in field_counts.items() if count > 1]:
            return [
                checks.Error(
                    ...
                    hint="Remove duplicates of %s." % ", ".join(map(repr, duplicate_fields)),
                    obj=obj.__class__,
                    id="admin.E006",
                ),
            ]
    ...
    def _check_fieldsets_item(self, obj, fieldset, label, seen_fields):
        ...
        if duplicate_fields := [
            field for field, count in field_counts.items() if count > 1 and field in fieldset_fields_set
        ]:
            return [
                checks.Error(
    ...
    def _check_radio_fields_value(self, obj, val, label):
        ...
        from django_adminx.options import HORIZONTAL, VERTICAL
    ...
    def _check_prepopulated_fields_key(self, obj, field_name, label):
        ...
        try:
            ...
            if isinstance(
                field,
                (models.DateTimeField, models.ForeignKey, models.ManyToManyField),
            ):
                return [
                    checks.Error(
...
class ModelAdminChecks(BaseModelAdminChecks):
    ...
    def _check_inlines_item(self, obj, inline, label):
        ...
        from django_adminx.options import InlineModelAdmin
        ...
        if not _issubclass(inline.model, models.Model):
            return must_be(
                "a Model",
    ...
    def _check_list_display_links(self, obj):
        ...
        from django_adminx.options import ModelAdmin
    ...
    def _check_list_filter_item(self, obj, item, label):
        ...
        """
        from django_adminx import FieldListFilter, ListFilter
    ...
    def _check_list_editable_item(self, obj, field_name, label):
        try:
            ...
            if obj.list_display[0] == field_name and not obj.list_display_links and obj.list_display_links is not None:
                return [
                    checks.Error(
